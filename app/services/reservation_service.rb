...
class ReservationService
  ...
  def rebuild
    ...
    reservations.each do |reservation|
      ...
      unless params[:start_at].nil? && params[:start_at].nil?
        reservation.start_at = "#{params[:date]} #{params[:start_at]}".to_datetime.strftime('%Y-%m-%dT%H:%M:%S')
        reservation.end_at = "#{params[:date]} #{params[:end_at]}".to_datetime.strftime('%Y-%m-%dT%H:%M:%S')
      end
      unless params[:start_at].nil? && params[:start_at].nil?
        reservation.start_at = "#{params[:date]} #{params[:start_at]}".to_datetime.strftime('%Y-%m-%dT%H:%M:%S')
        reservation.end_at = "#{params[:date]} #{params[:end_at]}".to_datetime.strftime('%Y-%m-%dT%H:%M:%S')
  ...
  def commit
    @reservation_density.save if !@reservation_density.nil? && @reservation_density.valid?
  ...
  def reservation_not_available
    ...
    range_dates.each do |d|
      start_at = "#{d} #{office.start_at}".to_datetime.strftime('%Y-%m-%dT%H:%M:%S')
      end_at = "#{d} #{office.end_at}".to_datetime.strftime('%Y-%m-%dT%H:%M:%S')
    ...
    if hours_not_available.empty?
      []
    else
      not_available = hours_not_available.first.count.positive? ? hours_not_available.first : hours_not_available.last
