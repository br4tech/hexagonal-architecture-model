class ClientsController < ApplicationController
  ...
  def combo   
    ...
    if @contract.new_record? 
      @contract.client = @client.present? ? @client : Client.new
  ...
  def search
    ...
    render json: @clients.as_json(only: [:id, :name, :email, :document, :zipcode, :street, :number, :neighborhood, :city, :state], methods: [:value])
  end
